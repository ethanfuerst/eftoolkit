...
`S3FileSystem` falls back to environment variables when kwargs are omitted.
Per field, precedence is:

**1. explicit kwarg** → **2. `S3_*` env var** → **3. `AWS_*` env var**

| Field | `S3_*` (primary) | `AWS_*` (fallback) |
|-------|------------------|--------------------|
...
| `endpoint` | `S3_ENDPOINT` | _(no fallback)_ |
...
!!! warning "Env vars not consulted"
    ...
    - `AWS_DEFAULT_REGION` — pass `region=...` or set `AWS_REGION` / `S3_REGION`.
