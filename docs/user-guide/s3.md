...
`S3FileSystem` falls back to AWS-standard environment variables when
kwargs are omitted. Per field, precedence is:

**1. explicit kwarg** → **2. AWS-standard env var**
...
s3 = S3FileSystem(region='eu-west-1')
```

If neither `access_key_id` nor `secret_access_key` can be resolved,
`S3FileSystem.__init__` raises `ValueError`.
...
!!! warning "Migration from `S3_*` env vars"
    ...
    AWS-standard equivalents above (`AWS_ACCESS_KEY_ID`,
    `AWS_SECRET_ACCESS_KEY`, `AWS_REGION`, `AWS_ENDPOINT_URL_S3`).

!!! warning "Env vars not consulted"
    The following are common in the AWS ecosystem but are **not** read by
    `S3FileSystem` today:

    - `S3_URL_STYLE` — URL style is only configurable via `DuckDB`'s
      `s3_url_style` kwarg.
    - `AWS_SESSION_TOKEN`, `AWS_PROFILE` — for session credentials or
      profile-based auth, construct a `boto3.session.Session` yourself and
      pass the resulting client through your own code path.
...
!!! note "Return value"
    `write_df_to_parquet` returns `None`. Row count is `len(df)`; the
    destination URI is the one you passed in. For the object's `ETag` or
    other put-response metadata, call `boto3`'s `head_object` directly
