...
- **Shared S3 credential resolution**: `_resolve_s3_credentials` (module-private, `eftoolkit/s3/filesystem.py`) centralizes the `kwarg → AWS_*` precedence used by both `S3FileSystem.__init__` and `DuckDB.__init__`. Env vars consulted: `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, `AWS_ENDPOINT_URL_S3`. For `region`, the env-var step is itself a chain: `AWS_REGION` then `AWS_DEFAULT_REGION` (matches boto3). `DuckDB()` auto-configures both its internal `S3FileSystem` and the native `CREATE SECRET` path from env vars. `DuckDB.s3_url_style` falls back to `S3_URL_STYLE` (ETH-439); fallback is inline in `DuckDB.__init__`, not in `_resolve_s3_credentials` (the helper is shared with `S3FileSystem`, which has no `s3_url_style` concept). Legacy `S3_*` env vars (`S3_ACCESS_KEY_ID`, `S3_SECRET_ACCESS_KEY`, `S3_REGION`, `S3_ENDPOINT`) are no longer read (ETH-429). Still not consulted: `AWS_SESSION_TOKEN`, `AWS_PROFILE`.
- **`clear_s3_env` fixture** (`tests/conftest.py`): pops the 5 `AWS_*` env vars (`AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, `AWS_REGION`, `AWS_DEFAULT_REGION`, `AWS_ENDPOINT_URL_S3`) for the test and restores on teardown. Use it in any test that asserts "no S3 configured" or sets its own env vars — otherwise tests flake on hosts with AWS creds in the environment.
- **PEP 561 typing**: `eftoolkit/py.typed` advertises inline type hints to consumer IDEs / type checkers (ETH-428). Hatchling ships it automatically via the default `packages = ["eftoolkit"]` wheel target — no explicit `force-include` entry in `pyproject.toml`. The marker applies recursively, so no per-subpackage markers are needed.
