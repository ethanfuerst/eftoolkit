...
- **Shared S3 credential resolution**: `_resolve_s3_credentials` (module-private, `eftoolkit/s3/filesystem.py`) centralizes the `kwarg → AWS_*` precedence used by both `S3FileSystem.__init__` and `DuckDB.__init__`. Env vars consulted: `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, `AWS_ENDPOINT_URL_S3`. For `region`, the env-var step is itself a chain: `AWS_REGION` then `AWS_DEFAULT_REGION` (matches boto3). `DuckDB()` auto-configures both its internal `S3FileSystem` and the native `CREATE SECRET` path from env vars. Legacy `S3_*` env vars (`S3_ACCESS_KEY_ID`, `S3_SECRET_ACCESS_KEY`, `S3_REGION`, `S3_ENDPOINT`) are no longer read (ETH-429). Still not consulted: `S3_URL_STYLE`, `AWS_SESSION_TOKEN`, `AWS_PROFILE`. `s3_url_style` remains kwarg-only.
