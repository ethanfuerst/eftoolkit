...
- **Deferred callable values**: any cell inside a `write_values` 2-D list may be a zero-arg callable. `Worksheet._resolve_value_updates` is called inside both `_flush_to_api` and `_flush_to_preview` immediately before the payload is sent/rendered. Callables stored inside a DataFrame passed to `write_dataframe` do **not** work — `df.values.tolist()` materializes at queue time.
- **`DashboardRunner.credentials`**: `dict[str, Any] | None = None`. Constructor raises `ValueError` if `credentials is None and not local_preview` — failure now surfaces at init, not inside phase 1.
